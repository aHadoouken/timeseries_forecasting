...
class Trainer:
    ...
    def plot_sample_predictions(self, epoch: int, n_samples: int = 3):
        ...
        for i in range(n_samples):
            ...
            axes[i, 0].plot(range(len(features[i]), len(features[i]) + len(targets[i])),
                           targets[i, :, 0], label='Target', color='green')
            axes[i, 0].plot(range(len(features[i]), len(features[i]) + len(predictions[i])),
                           predictions[i, :, 0], label='Prediction', color='red', linestyle='--')
            ...
            axes[i, 1].plot(range(len(features[i]), len(features[i]) + len(targets[i])),
                           targets[i, :, 1], label='Target', color='green')
            axes[i, 1].plot(range(len(features[i]), len(features[i]) + len(predictions[i])),
                           predictions[i, :, 1], label='Prediction', color='red', linestyle='--')
            axes[i, 1].set_title(f'Sample {i+1} - Velocity')
            axes[i, 1].legend()
            axes[i, 1].grid(True)

        plt.tight_layout()
        plt.savefig(os.path.join(self.output_dir, f'predictions_epoch_{epoch}.png'), dpi=150)
        plt.close()
